...
# - axel, wget, or curl is needed for downloading.
...
w_download_to()
{
    ...
    do
        ...
        *) _W_agent= ;;
        ...
        if [ -x "`which axel 2>/dev/null`" ]
        then
            # Basic axel support.
            axel -o "$_W_file" "$_W_url"
...
load_dotnet30sp1()
{
    ...
    *) w_warn "dotnet30sp1 does not yet fully work or install on wine.  Caveat emptor." ;;
    ...
    w_download http://download.microsoft.com/download/2/5/2/2526f55d-32bc-410f-be18-164ba67ae07d/XPSEP%20XP%20and%20Server%202003%2032%20bit.msi 5d332ebd1025e294adafe72030fe33db707b2c82
...
load_vcrun2008()
{
    ...
    w_override_dlls native,builtin msvcm90 msvcp90 msvcr90 vcomp90
...
w_metadata cmake apps \
    ...
    year="2011" \
    media="download" \
    file1="cmake-2.8.6-win32-x86.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/CMake 2.8/bin/cmake-gui.exe"

load_cmake()
{
    w_download http://www.cmake.org/files/v2.8/cmake-2.8.6-win32-x86.exe 47172b78e5d5f99cf5dc623db309781234153d56
    cd "$W_CACHE"/cmake
    w_try $WINE cmake-2.8.6-win32-x86.exe $W_UNATTENDED_SLASH_S
...
load_uplay()
{
    ...
    w_download http://static3.cdn.ubi.com/orbit/launcher_installer/UplayInstaller.exe 21a7f469534dd1463eaaab6b2be2fa9084bceea2
    cd "$W_CACHE"/uplay
...
load_simsmed()
{
    ...
    then
        myexec="Exec=env WINEPREFIX=\"$WINEPREFIX\" wine cmd /c 'C:\\\\\\\\Run-$W_PACKAGE.bat'"
        mymenu="$HOME/.local/share/applications/wine/Programs/Electronic Arts/The Sims Medieval/The Simsâ„¢ Medieval.desktop"
        if test -f "$mymenu"
        then
            sed -i "s,Exec=.*,$myexec," "$mymenu"
        fi
