...
def init_logging():
    ...
    fileHandler = logging.FileHandler(filename = './autocrop.log')
...
def parse_commandline():
    ...
    #    ./autocrop.py --inputdir ${SHAREDIR} --outputdir ${SHARE}/cropped --serialize
    parser.add_argument('--inputdir', dest='inputdir', help='input directory', required=True)
    parser.add_argument('--outputdir', dest='outputdir', help='output directory', required=True)
    parser.add_argument('--serialize', dest='serialize', action='store_true', default=False, help='serialized operation')
    parser.add_argument('--onefile', dest='onefile', action='store_true', help='Process one file', default=False)
    parser.add_argument('--format', dest='format', help='Output Format', default='bmp', choices = ['png', 'bmp'])
...
def threadrun(inputdir, outputdir, file):
#    outfilename='{:08d}.{}'.format(int(os.path.basename(file)[:-5])+200, config.format)
#    outfile = '{dir}/{filename}'.format(dir=todir, filename = outfilename)
    outfile=f'{outputdir}/{file}'
    ...
    image = Image.open(f'{inputdir}/{file}')
...
def mainthreaded():
    ...
    while True:
        ...
        if not os.path.exists(config.inputdir):
            logger.error(f'{config.inputdir} does not exist')
            return 1
