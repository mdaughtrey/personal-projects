...
mkdir -p $(dirname $OUT)
for ff in ${OUT}/*.png; do convert $ff -resize 2160x1458\! $ff;  done
docker run --rm -h ${NAME} --name ${NAME} -t -v ${FRAMES}:/frames --entrypoint /ffmpeg/ffmpeg  ${NAME}:latest  -i /frames/$(basename ${SCRIPT}) -y ${OUT}
