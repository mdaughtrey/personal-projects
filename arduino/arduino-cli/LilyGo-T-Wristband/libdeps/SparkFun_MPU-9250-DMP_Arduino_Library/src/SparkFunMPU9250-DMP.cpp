...
inv_error_t MPU9250_DMP::dmpBegin(unsigned short features, unsigned short fifoRate, char * buff)
{
	unsigned short feat = features;
	unsigned short rate = fifoRate;

    Serial.println("dmpBegin");
	if (dmpLoad(buff) != INV_SUCCESS)
        return INV_ERROR;
    Serial.println("dmpLoad");
    ...
    Serial.println("dmpEnableFeatures");
    ...
    Serial.println("dmpSetFifoRate");
	
    int rc =  mpu_set_dmp_state(1);
    Serial.println(String("mpu_set_dmp_state ") + String(rc));
    return rc;
}

inv_error_t MPU9250_DMP::dmpLoad(char * buff)
{
    return dmp_load_motion_driver_firmware(buff);
}
