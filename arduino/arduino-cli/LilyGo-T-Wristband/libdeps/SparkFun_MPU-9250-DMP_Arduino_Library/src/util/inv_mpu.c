...
int mpu_write_mem2(unsigned short mem_addr, unsigned short length,
        unsigned char *data, char * buff)
{
    unsigned char tmp[2];
    char buf2[32];
    sprintf(buf2, "Writing to %02x\n", st.hw->addr);
    strcat(buff, buf2);
...
int mpu_load_firmware(unsigned short length, const unsigned char *firmware,
    unsigned short start_addr, unsigned short sample_rate, char * rc)
{
    char buff[256];
    unsigned short ii;
...
#define LOAD_CHUNK  (16)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        sprintf(buff, "Writing: %08x\r\n", ii);
        strcat(rc, buff);
        this_write = min(LOAD_CHUNK, length - ii);
        if (mpu_write_mem2(ii, this_write, (unsigned char*)&firmware[ii], rc))
        {
            sprintf(buff, "mpu_write_mem length %d ii %d\r\n", length, ii);
            strcat(rc, buff);
            return -1;
        }
        if (mpu_read_mem(ii, this_write, cur))
        {
            sprintf(buff, "mpu_read_mem ii %d\r\n", ii);
            strcat(rc, buff);
            return -1;
        }
        if (memcmp(firmware+ii, cur, this_write))
        {
            ...
            {
                sprintf(buff, "%04x:%02x ", (int)(jj+ii), cur[jj]);
                strcat(rc, buff);
            }
            strcat(rc, "\r\nwritten: ");
            for (int jj = 0; jj < this_write; jj++)
            {
                sprintf(buff, "%04x:%02x ", (int)(jj+ii), *(unsigned char *)(firmware+jj+ii));
                strcat(rc, buff);
            }
            strcat(rc, "\r\n");
    ...
    strcat(rc, "mpu_load_firmware done\r\n");
    return 0;
}
