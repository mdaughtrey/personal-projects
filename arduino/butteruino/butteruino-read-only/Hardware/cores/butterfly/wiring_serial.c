...
void beginSerial(long baud)
{
    ...
    sbi(UCSRA, U2X );    
	UBRRH = ((F_CPU /  8 + baud / 2) / baud - 1) >> 8;
	UBRRL = ((F_CPU /  8 + baud / 2) / baud - 1);
#else
    // For higher clock speeds clear U2X for normal speed operation 
    // Baud rate error should be kept to +/- 2% in this mode 
    cbi(UCSRA, U2X);
